...
export interface CompanyInput { legalName: string; tradeName?: string; countryCode: string; taxId: string; phone?: string; contactEmail: string; description?: string; logoUrl?: string }
...
export async function publicCompany(id: string) { const company = await prisma.company.findUnique({ where: { id }, select: { id: true, legalName: true, tradeName: true, countryCode: true, taxId: true, description: true, logoUrl: true, verificationStatus: true, verifiedAt: true, createdAt: true } }); if (!company) throw new AppError("company not found", 404); return company; }
